...
import { defaultComponents } from './portableText.defaults';
import { mergeComponents } from './portableText.merge';
import type {
...
const getNodeRenderer = (
  ...
  function renderNode<N extends TypedObject>(
    options: Serializable<N>,
  ): ReactNode {
  ...
  function renderList(node: ReactPortableTextList, index: number, key: string) {
    const children = node.children.map((child, childIndex) =>
      renderNode({
        node: child._key
          ? child
          : { ...child, _key: `li-${index}-${childIndex}` },
        index: childIndex,
        isInline: false,
        renderNode,
      }),
