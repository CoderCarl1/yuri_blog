...
export default async function patchSanityDocument<
  QueryResponse = SanityDocument,
>(
  ...
  if (!data || Object.keys(data).length === 0)
    throw new Error('Data must be provided to update the document');
...
export async function createSanityDocument<QueryResponse = SanityDocument>(
  ...
  if (!sanityDocumentId) throw new Error('The documentID must be provided');
  if (!data || Object.keys(data).length === 0)
    throw new Error('Data must be provided to create the document');
